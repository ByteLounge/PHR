...
export const ParkerScrollytelling: React.FC<{
  ...
  const imagesRef = useRef<(HTMLImageElement | null)[]>(new Array(TOTAL_PARKER_FRAMES + 1).fill(null));
  ...
  useEffect(() => {
    ...
    const loadIndex = (index: number): Promise<void> => {
      return new Promise((resolve) => {
        ...
        img.src = getParkerFramePath(index);
        img.onload = () => {
          if (!isCancelled) {
    ...
    Promise.all(priorityIndices.map((i) => loadIndex(i))).then(() => {
      ...
      for (let i = 1; i <= TOTAL_PARKER_FRAMES; i++) {
  ...
  const drawFrame = useCallback((frameIdx: number) => {
    ...
    const clampedIdx = Math.max(1, Math.min(TOTAL_PARKER_FRAMES, Math.round(frameIdx)));
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      targetFrameRef.current = Math.max(1, Math.min(TOTAL_PARKER_FRAMES, calculatedFrame));
    };
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      targetFrameRef.current = (targetFrameRef.current % TOTAL_PARKER_FRAMES) + 1;
    }, 40);
  ...
  const scrollToShop = () => {
    if (onShopClick) {
      ...
      const details = document.getElementById("parker-details");
